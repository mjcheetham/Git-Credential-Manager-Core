...
namespace Microsoft.Git.CredentialManager
{
    ...
    public class Settings : ISettings
    {
        ...
        public ProxyConfiguration GetProxyConfiguration()
        {
            ...
            // The NO_PROXY environment variable is the only way to specify bypassed hosts in all situations,
            // so try and read that first so we can apply it to all possible proxy configurations.
            string bypassHosts = null;
            if (TryGetSetting(KnownEnvars.CurlNoProxy, null, null, out string noProxy))
            {
                bypassHosts = noProxy;
            }
            ...
            {
                return new ProxyConfiguration(proxyUri, bypassHosts, true);
            }

            // 2. Standard Git configuration
            if (TryGetUriSetting(null, GitHttpCfg.SectionName, GitHttpCfg.Proxy, out proxyUri))
            {
                return new ProxyConfiguration(proxyUri, bypassHosts, false);
            }
