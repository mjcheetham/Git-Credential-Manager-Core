        ...
        private int ListCmd(string organization, bool showRemotes, bool verbose)
        {
            ...
            foreach (string orgName in orgNames)
            {
                ...
                foreach (var remoteBinding in remoteBindingMap)
                {
                    ...
                    {
                        // User names in dev.azure.com URLs cannot be used as *actual user names*
                        // because of the ..interesting.. decision to use this field to get the
                        // Azure DevOps organization name to be send by Git to credential helpers.
                        //
                        // We always show dev.azure.com URLs as "inherit", even if there is a username
                        // in the URL.
                        if (!binding.Uri.TryGetUserInfo(out string userName, out _) ||
                            UriHelpers.IsDevAzureComHost(binding.Uri.Host))
                        {
                            userName = "(inherit)";
                        }
